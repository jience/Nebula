...
import { Monitor, AppWindow, Play, Clock, MoreVertical, Star, X, Cpu, Zap, Globe, Server, Info, History, Power, PlugZap, Search, Signal, Wifi, Laptop, Command, PowerOff, RotateCcw, AlertTriangle, Loader2, CheckCircle2, Trash2, Filter, Users, Shield, User, RefreshCw, Terminal, Layers, Box, LayoutGrid } from 'lucide-react';
import { VDIResource, ResourceType, ResourceSubType, ActivityLogEntry } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ onLaunch, category, searchQuery }) => {
  ...
  const ResourceCard: React.FC<{ resource: VDIResource, large?: boolean }> = ({ resource, large = false }) => {
    ...
    return (
        <div 
          ...
          className={`group relative bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl transition-all duration-300 hover:shadow-xl ${styles.border} ${styles.shadow} overflow-hidden flex flex-col cursor-pointer ${large ? 'col-span-1 md:col-span-2 row-span-2' : ''} ${isProcessing ? 'pointer-events-none opacity-90' : ''}`}
        >
  ...
  return (
    <div className="relative space-y-8 animate-fade-in pb-20">
      ...
      {/* Activity Log Sidebar - Moved to end of DOM for better natural stacking and increased z-index */}
      <div 
        className={`fixed top-14 right-0 bottom-8 w-80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-l border-slate-200 dark:border-slate-700 shadow-2xl z-40 transform transition-transform duration-300 ease-in-out flex flex-col ${showActivityLog ? 'translate-x-0' : 'translate-x-full'}`}
      >
